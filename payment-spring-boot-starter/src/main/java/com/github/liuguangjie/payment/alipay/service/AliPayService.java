package com.github.liuguangjie.payment.alipay.service;
...
public class AliPayService extends BasePayService<AliPayConfigStorage> {
     ...
     * 回调校验
     *
     * @param requestParams 回调回来的参数集
     * @return 签名校验 true通过
     */
    @Override
    public boolean verify(Map<String, Object> requestParams) {
        ...
        boolean flag = false;
        try {
            flag = AlipaySignature.rsaCheckV1(params, payConfigStorage.getKeyPublic(), payConfigStorage.getInputCharset(),
                    payConfigStorage.getSignType());
    ...
    public Map<String, Object> orderInfo(PayOrder order) {
        String orderInfo = "";
        //实例化客户端
        AlipayClient alipayClient = new DefaultAlipayClient(HTTPS_REQ_URL, payConfigStorage.getAppid(),
                payConfigStorage.getKeyPrivate(), "json", payConfigStorage.getInputCharset(),
                payConfigStorage.getKeyPublic(), payConfigStorage.getSignType());
        ...
        model.setTotalAmount(Util.conversionAmount(order.getPrice()).toString());
        model.setProductCode("QUICK_MSECURITY_PAY");
        request.setBizModel(model);
        request.setNotifyUrl(payConfigStorage.getNotifyUrl());
        try {
